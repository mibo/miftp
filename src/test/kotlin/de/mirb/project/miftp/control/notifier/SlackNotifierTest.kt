...
class SlackNotifierTest {
  ...
  fun initFilterEmpty() {
    ...
    notifier.init(mapOf("slack_webhook_url" to "http://"), mockk<FtpFileStore>())
  ...
  fun initFilterSuccess() {
    val notifier = SlackNotifier()
    notifier.init(mandatoryInitParameters.plus("event_filter" to "CREATED, DELETED"), mockk<FtpFileStore>())
    //val contains = notifier.filter.containsAll(listOf(FileSystemEvent.EventType.CREATED, FileSystemEvent.EventType.DELETED))
